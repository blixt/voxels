...
struct PendingRelocation {
    ...
    profile_after_commit: Option<voxels_core::ProfileAutomation>,
    destination_resolved: bool,
}

#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
#[cfg(any(target_arch = "wasm32", test))]
struct RelocationReadiness {
    presentation_envelope_committed: bool,
    destination_presentable: bool,
    canonical_destination_ready: bool,
}

#[cfg(any(target_arch = "wasm32", test))]
impl RelocationReadiness {
    const fn is_ready(self) -> bool {
        self.presentation_envelope_committed
            && self.destination_presentable
            && self.canonical_destination_ready
    }
}

/// Completes an exceptional camera handoff once, and only once, after every destination product
/// has certified the same target. A caller retains the unchanged source camera while this returns
/// `None`; consuming the transaction is the atomic handoff.
#[cfg(any(target_arch = "wasm32", test))]
fn take_ready_relocation(
    pending: &mut Option<PendingRelocation>,
    readiness: RelocationReadiness,
) -> Option<PendingRelocation> {
    (readiness.is_ready()
        && pending
            .as_ref()
...
#[cfg(any(target_arch = "wasm32", test))]
fn merge_enclosed_view_stream_plans(
    mut source: EnclosedViewStreamPlan,
    destination: EnclosedViewStreamPlan,
) -> EnclosedViewStreamPlan {
    source.chunks.extend(destination.chunks);
    source
        .chunks
        .sort_unstable_by_key(|coord| (coord.x, coord.y, coord.z));
    source.chunks.dedup();
    source.frontiers.extend(destination.frontiers);
    source.frontiers.sort_unstable_by_key(|frontier| {
        (
            frontier.source.x,
            frontier.source.y,
            frontier.source.z,
...
mod web {
    ...
    fn virtual_terrain_upload_failure_kind(error: &VirtualTerrainRendererError) -> u8 {
        match error {
            ...
            VirtualTerrainRendererError::GpuSnapshot => 7,
            VirtualTerrainRendererError::SelectedCutSnapshotCapacity { .. } => 8,
    ...
    impl Engine {
        ...
        fn profile_step_relocation(
            &self,
            source: CameraState,
            profile_after_commit: ProfileAutomation,
        ) -> Option<crate::PendingRelocation> {
            let pose = profile_after_commit.admission_pose()?;
            ...
            let surface = resident_surface_sample(
                &self.chunks.borrow(),
                voxel_x,
                voxel_z,
                self.remote_environment.1,
            );
            let mut position = Vec3::new(pose.position_xz.x, source.position.y, pose.position_xz.y);
            if let Some(surface) = surface {
                let top = surface
                    .water_level
                    .unwrap_or(surface.height)
                    .max(surface.height);
                position.y = (top + 1) as f32 * VOXEL_SIZE_METRES
                    + voxels_core::PLAYER_EYE_HEIGHT_METRES
                    + 0.8;
            }
            let mut destination = CameraState::spawn(position);
            destination.velocity = Vec3::new(pose.velocity_xz.x, 0.0, pose.velocity_xz.y);
            destination.yaw = pose.yaw;
            destination.pitch = pose.pitch;
            Some(crate::PendingRelocation {
                kind: crate::PendingRelocationKind::ProfileStep,
                destination,
                presentation_target: position,
                profile_after_commit: Some(profile_after_commit),
                destination_resolved: surface.is_some(),
            })
        }

        fn refresh_profile_relocation(
            &self,
            mut relocation: crate::PendingRelocation,
            ...
            let voxel_x = (relocation.destination.position.x / VOXEL_SIZE_METRES).floor() as i32;
            let voxel_z = (relocation.destination.position.z / VOXEL_SIZE_METRES).floor() as i32;
            let Some(surface) = resident_surface_sample(
                &self.chunks.borrow(),
                voxel_x,
                voxel_z,
                self.remote_environment.1,
            ...
            let top = surface
                ...
                .unwrap_or(surface.height)
                .max(surface.height);
            relocation.destination.position.y =
                (top + 1) as f32 * VOXEL_SIZE_METRES + voxels_core::PLAYER_EYE_HEIGHT_METRES + 0.8;
            relocation.presentation_target.y = relocation.destination.position.y;
            relocation.destination_resolved = true;
            relocation
        }

        fn relocation_canonical_interests(
            &self,
            destination: &CameraState,
            ...
            (
                ...
                self.enclosed_view_stream_interest(destination),
            )
        }

        fn relocation_readiness(
            &self,
            relocation: crate::PendingRelocation,
            envelope: &PresentationEnvelope,
        ) -> crate::RelocationReadiness {
            let (collision_interest, enclosed_interest) =
                self.relocation_canonical_interests(&relocation.destination);
            let canonical_destination_ready = relocation.destination_resolved && {
                let scheduler = self.scheduler.borrow();
                scheduler.interest_readiness(&collision_interest).is_ready()
                    && (enclosed_interest.is_empty()
                        || scheduler.interest_readiness(&enclosed_interest).is_ready())
            };
            let renderer = self.renderer.borrow();
            crate::RelocationReadiness {
        ...
        fn frame(&self, time: f64) {
            ...
            *camera = self.client_view.borrow().camera();
                ...
                && steps < self.config.max_steps_per_frame
            {
                if profiling {
                    if !matches!(
                        self.profile.borrow().phase(),
                        ProfilePhase::Warmup | ProfilePhase::Measured
                    ) {
                        // Drain owns an already admitted terminal pose. It waits for the exact
                        // render/canonical certificate below and consumes no additional ticks.
                        accumulator = 0.0;
                        break;
                    }
                    let mut next_profile = *self.profile.borrow();
                    ...
                    if let Some(relocation) = self.profile_step_relocation(*camera, next_profile) {
                        ...
                        // Idle/Complete have no route target. Running phases, including the
                        // terminal Drain transition, always take the transactional branch above.
                        *self.profile.borrow_mut() = next_profile;
                    }
                } else {
            ...
            if profiling {
                ...
                // motion intent explicitly through the production velocity-lookahead path. Once
                // the last pose is admitted, Drain is stationary.
            ...
            let source_streaming_velocity = if profiling || reproducing {
                camera.velocity
            } else {
        ...
        fn stream_world(
            ...
            // Canonical chunks are simulation and collision data only. During an exceptional
            // handoff, source and destination interests coexist while the scheduler focus moves to
            // the destination. The visible A cut remains committed until B's full envelope is
            // publishable, and B cannot become the camera until its exact canonical corridor is
            // resident.
            ...
            let mesh_start = performance_now(performance);
            {
                ...
                for ticket in work.meshing {
                    ...
                    let Some(surface) = surfaces.get(&coord_key(ticket.coord)) else {
                        let _ = self.scheduler.borrow_mut().retry(ticket);
                        continue;
                    };
        ...
        fn stream_virtual_terrain(
            ...
            {
                log_gpu_error(&format!("hydrate cached virtual terrain: {error}"));
                return;
            }
            ...
            if !relevant_revision_pending {
                ...
                // Continue fetching them while a frozen publication awaits certification; only
            ...
            if let Err(error) = self
                ...
                .borrow_mut()
                .reconcile_with_available(demands, cached_demands)
            {
        ...
        fn hydrate_virtual_terrain_from_cache(
            &self,
            ...
            let mut remaining_pages = VIRTUAL_TERRAIN_CACHE_UPLOADS_PER_FRAME;
            let mut remaining_bytes = VIRTUAL_TERRAIN_CACHE_UPLOAD_BYTES_PER_FRAME;
            let mut uploaded = false;
            loop {
                if remaining_pages == 0
                    || remaining_bytes == 0
                    || js_sys::Date::now() - started_ms >= VIRTUAL_TERRAIN_CACHE_UPLOAD_CPU_MS
                {
                    break;
                }
                let requested = cut.requested_pages.iter().copied().collect::<BTreeSet<_>>();
                self.virtual_terrain
                    .borrow_mut()
                    .page_upload_retry_after_ms
                    .retain(|identity, _| requested.contains(identity));
                ...
                for group in groups {
                    ...
                    let identities = group
                        ...
                        .collect::<Vec<_>>();
                    if identities.len() > remaining_pages
                ...
                let result = if let Some(parent) = parent {
                    self.renderer
                        .borrow_mut()
                        .upload_virtual_terrain_replacement_group(parent, pages)
                } else {
                    debug_assert_eq!(pages.len(), 1);
                    let Some(page) = pages.into_iter().next() else {
                        break;
                    };
                    self.renderer.borrow_mut().upload_virtual_terrain_page(page)
                };
                match result {
                    Ok(()) => {
                        let mut state = self.virtual_terrain.borrow_mut();
                        for identity in &identities {
                            state.page_upload_retry_after_ms.remove(identity);
                        }
                    }
                    Err(error) => {
                        let mut state = self.virtual_terrain.borrow_mut();
                        state.stats.page_upload_failed = state
                            .stats
                            .page_upload_failed
                            .saturating_add(identities.len() as u64);
                        state.stats.last_page_upload_failure_kind =
                            virtual_terrain_upload_failure_kind(&error);
                        // Capacity is a current admission fact. A timed retry hides progress made by
                        // eviction or publication and can stall the exact group after space opens.
                        if !matches!(error, VirtualTerrainRendererError::GpuPoolCapacity) {
                            for identity in &identities {
                                state.page_upload_retry_after_ms.insert(
                                    *identity,
                                    now_ms.saturating_add(VIRTUAL_TERRAIN_DIRECTORY_RETRY_MS),
                                );
                            }
                        ...
                        drop(state);
                        log_gpu_error(&format!(
                            "admit cached virtual terrain group {:?}: {error}",
                            parent.or_else(|| identities.first().and_then(|identity| {
                                identity.key.parent().or(Some(identity.key))
                            }))
                        ));
                        // Preserve any earlier admitted groups and let a legal current cut freeze.
                        // Returning from the whole stream here would retry the blocker forever
                        // before publication or transport reconciliation could make progress.
                        break;
                    }
                }
                remaining_pages = remaining_pages.saturating_sub(identities.len());
                remaining_bytes = remaining_bytes.saturating_sub(encoded_bytes);
                uploaded = true;
                // Every admitted group can alter balance closure. Reselect immediately so the next
                // group is ranked against the true legal frontier, not a stale request list.
                *cut = self
                    .renderer
                    .borrow_mut()
                    .select_virtual_terrain_cut(view, presentation_envelope)?;
        ...
        fn accept_remote_terrain_page_completion(&self, completion: RemoteTerrainPageCompletion) {
            ...
            for item in result.batch.items {
                ...
                let useful = self
                    ...
                    .unwrap_or(false);
                if !useful {
                    // The exact identity became cache-resident while this transfer was in flight.
                    // Do not refresh/repopulate it from an obsolete response after accounting the
                    // duplicate bytes as cancellation waste.
                    continue;
                }
                if let Err(error) = self
    ...
    impl EngineHandle {
        ...
        pub fn startup_progress(&self) -> Vec<u32> {
            ...
            let column_timed_out = virtual_state.stats.column_timed_out;
            let column_other_failed = virtual_state.stats.column_other_failed;
            let directory_accepted = virtual_state.stats.directory_accepted;
            let directory_submit_deferred = virtual_state.stats.directory_submit_deferred;
            let directory_preempted = virtual_state.stats.directory_preempted;
            let directory_timed_out = virtual_state.stats.directory_timed_out;
            let directory_other_failed = virtual_state.stats.directory_other_failed;
            ...
            let progress = vec![
                ...
                render.virtual_terrain_gpu_encoded_pages,
                render.virtual_terrain_gpu_ownerless_roots,
                render.virtual_terrain_gpu_encoding_overflow_flags,
                u32::from(render.virtual_terrain_presented_snapshot_matches_cut),
                ...
                u32::try_from(column_timed_out).unwrap_or(u32::MAX),
                u32::try_from(column_other_failed).unwrap_or(u32::MAX),
                u32::try_from(directory_accepted).unwrap_or(u32::MAX),
                u32::try_from(directory_submit_deferred).unwrap_or(u32::MAX),
                u32::try_from(directory_preempted).unwrap_or(u32::MAX),
                u32::try_from(directory_timed_out).unwrap_or(u32::MAX),
                u32::try_from(directory_other_failed).unwrap_or(u32::MAX),
        ...
        pub fn snapshot(&self) -> Vec<f32> {
            ...
            if let Some(engine) = self.engine.as_ref() {
                ...
                let (
                    ...
                    virtual_terrain_page_submit_deferred,
                    virtual_terrain_page_preempted,
                    virtual_terrain_page_timed_out,
                    virtual_terrain_page_other_failed,
                    virtual_terrain_page_unavailable,
                    virtual_terrain_page_stale_revision,
                    virtual_terrain_page_generation_failed,
                    virtual_terrain_page_upload_failed,
                ) = {
                    ...
                    (
                        ...
                        state.stats.page_timed_out,
                        state.stats.page_other_failed,
                        state.stats.page_unavailable,
                        state.stats.page_stale_revision,
                        state.stats.page_generation_failed,
                ...
                values.extend_from_slice(&[
                    ...
                    render.virtual_terrain_gpu_encoding_overflow_flags as f32,
                    render.virtual_terrain_gpu_encoded_pages as f32,
                    ...
                    virtual_terrain_page_submit_deferred as f32,
                    virtual_terrain_page_preempted as f32,
                    virtual_terrain_page_timed_out as f32,
                    virtual_terrain_page_other_failed as f32,
                    virtual_terrain_page_unavailable as f32,
                    virtual_terrain_page_stale_revision as f32,
                    virtual_terrain_page_generation_failed as f32,
                    virtual_terrain_page_upload_failed as f32,
                    ...
                    (render.virtual_terrain_presented_snapshot_generation & 0x00ff_ffff) as f32,
                    ((render.virtual_terrain_presented_snapshot_generation >> 24) & 0x00ff_ffff)
                        as f32,
                    (render.virtual_terrain_presented_snapshot_fingerprint & 0x00ff_ffff) as f32,
                    ((render.virtual_terrain_presented_snapshot_fingerprint >> 24) & 0x00ff_ffff)
                        as f32,
                    if render.virtual_terrain_presented_snapshot_matches_cut {
                        1.0
                    } else {
                        0.0
                    },
...
mod tests {
    ...
    #[test]
    fn exceptional_relocation_holds_source_until_every_destination_product_is_ready() {
        ...
        let held_source = source.position;
        ...
        for readiness in [
            ...
            assert!(take_ready_relocation(&mut pending, readiness).is_none());
            assert!(
                pending.is_some(),
                "the destination intent must remain durable"
            );
            assert_eq!(
                source.position, held_source,
                "readiness polling must not mutate the visible source"
            );
        }
        ...
        .expect("the complete destination certificate commits the handoff");
        assert_eq!(committed.destination.position, destination.position);
        ...
        assert!(
            ...
            .is_none(),
            "a consumed handoff cannot oscillate or apply twice"
        );
    }

    #[test]
    fn unresolved_profile_surface_cannot_commit_despite_other_certificates() {
        let destination = CameraState::spawn(glam::Vec3::new(64.0, 8.0, 64.0));
        let mut relocation = pending_relocation(
            PendingRelocationKind::ProfileStep,
            destination,
            destination.position,
        );
        relocation.destination_resolved = false;
        let mut pending = Some(relocation);
        assert!(
            take_ready_relocation(
                &mut pending,
    ...
    #[test]
    fn relocation_stream_plan_retains_source_and_destination_without_duplicate_frontiers() {
        let shared_frontier = PortalFrontier {
            source: voxels_world::ChunkCoord::new(0, 1, 0),
            neighbor: voxels_world::ChunkCoord::new(1, 1, 0),
            face: 1,
            cells: [7; CHUNK_FACE_WORDS],
        };
        let merged = merge_enclosed_view_stream_plans(
            EnclosedViewStreamPlan {
                chunks: vec![voxels_world::ChunkCoord::new(0, 1, 0)],
                frontiers: vec![shared_frontier.clone()],
            },
            EnclosedViewStreamPlan {
                chunks: vec![
                    voxels_world::ChunkCoord::new(0, 1, 0),
                    voxels_world::ChunkCoord::new(8, 1, 0),
                ],
                frontiers: vec![shared_frontier],
            },
        );
        assert_eq!(
            merged.chunks,
        ...
        assert_eq!(merged.frontiers.len(), 1);
    }
