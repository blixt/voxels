...
interface CapturedFrame {
  ...
  readonly midpointMilliseconds: number;
}

interface RainMotionPair {
  readonly score: number;
  readonly dx: number;
  readonly dy: number;
  readonly dxPixels: number;
  readonly dyPixels: number;
  readonly velocityYPixelsPerSecond: number;
  readonly positiveAdvantage: number;
}
...
interface RainAnalysis {
  ...
  readonly pairs: readonly RainMotionPair[];
}

interface CameraLook {
  readonly yaw: number;
...
interface CloudAnalysis {
  readonly layering: {
    ...
    readonly horizontalCoherence: number;
    readonly verticalCoherence: number;
    readonly directionalExcess: number;
    readonly maximumRowAutocorrelation: number;
    readonly maximumRowAutocorrelationLagPixels: number;
    readonly maximumRowAutocorrelationProminence: number;
  };
  readonly rotation: {
    readonly samples: number;
    readonly weightedSamples: number;
    readonly meanAbsoluteError: number;
    readonly weightedMeanAbsoluteError: number;
    readonly weightedCorrelation: number;
    readonly catastrophicFraction: number;
  };
  readonly returned: {
    readonly meanAbsoluteError: number;
    readonly catastrophicFraction: number;
...
function wrappedFraction(value: number): number {
  return ((value % 1) + 1) % 1;
}
...
async function analyzeRainMotion(
  page: Page,
  ...
  return page.evaluate(async (serializedFrames) => {
    const at = (values: ArrayLike<number>, index: number): number => {
      ...
      if (value === undefined) throw new Error(`image analysis index ${index} is out of range`);
      return value;
    };
    const decoded: {
      width: number;
    ...
    for (const frame of serializedFrames) {
      ...
      if (imageContext === null) throw new Error("2D rain analysis context is unavailable");
      imageContext.drawImage(bitmap, 0, 0);
      decoded.push({
        width: bitmap.width,
        height: bitmap.height,
        pixels: imageContext.getImageData(0, 0, bitmap.width, bitmap.height).data,
        midpointMilliseconds: frame.midpointMilliseconds,
      });
    }
    const firstDecoded = decoded[0];
    if (firstDecoded === undefined) throw new Error("rain analysis received no frames");
    const step = 4;
    const x0 = Math.floor(firstDecoded.width * 0.08);
    const x1 = Math.floor(firstDecoded.width * 0.78);
    const y0 = Math.floor(firstDecoded.height * 0.05);
    const y1 = Math.floor(firstDecoded.height * 0.82);
    ...
    const lumaFrames = decoded.map((frame) => {
      ...
      for (let y = 0; y < height; y += 1) {
        for (let x = 0; x < width; x += 1) {
          ...
          const red = at(frame.pixels, source) / 255;
          const green = at(frame.pixels, source + 1) / 255;
          const blue = at(frame.pixels, source + 2) / 255;
    ...
    for (let index = 0; index < background.length; index += 1) {
      const values = lumaFrames
        .map((frame) => at(frame, index))
        .sort((left, right) => left - right);
      background[index] = at(values, Math.floor(values.length / 2));
    }
    const signals = lumaFrames.map((luma) => {
      ...
      for (let index = 0; index < luma.length; index += 1) {
        ...
        signal[index] = Math.max(Math.abs(at(luma, index) - at(background, index)) - 0.006, 0);
      }
      return signal;
    });
    const pairs: RainMotionPair[] = [];
    for (let frameIndex = 0; frameIndex + 1 < signals.length; frameIndex += 1) {
      ...
      if (first === undefined || second === undefined) {
        throw new Error("rain signal pair is incomplete");
      }
        ...
        for (let dx = -3; dx <= 3; dx += 1) {
          ...
          for (let y = 30; y < height - 30; y += 1) {
            for (let x = 3; x < width - 3; x += 1) {
              const left = at(first, x + y * width);
              const right = at(second, x + dx + (y + dy) * width);
              product += left * right;
      ...
      const currentFrame = decoded[frameIndex];
      const nextFrame = decoded[frameIndex + 1];
      if (currentFrame === undefined || nextFrame === undefined) {
        throw new Error("rain timing pair is incomplete");
      }
      const elapsedSeconds =
        (nextFrame.midpointMilliseconds - currentFrame.midpointMilliseconds) / 1_000;
      pairs.push({
        ...best,
...
async function analyzeCloudRotation(
  page: Page,
  baselineFrames: readonly CapturedFrame[],
  rotatedFrames: readonly CapturedFrame[],
  returnedFrames: readonly CapturedFrame[],
  cameras: { readonly baseline: CameraLook; readonly rotated: CameraLook },
  ...
  return page.evaluate(
    async ({
      ...
      const at = (values: ArrayLike<number>, index: number): number => {
        const value = values[index];
        if (value === undefined) throw new Error(`cloud analysis index ${index} is out of range`);
        return value;
      };
      async function medianLuma(frames: readonly CapturedFrame[]): Promise<LumaImage> {
        const decoded: {
          readonly width: number;
          readonly height: number;
        ...
        for (const frame of frames) {
          ...
          const canvas = new OffscreenCanvas(bitmap.width, bitmap.height);
          const imageContext = canvas.getContext("2d", { willReadFrequently: true });
          if (imageContext === null) throw new Error("2D cloud analysis context is unavailable");
          imageContext.drawImage(bitmap, 0, 0);
          decoded.push({
            width: bitmap.width,
            height: bitmap.height,
        ...
        if (first === undefined) throw new Error("cloud analysis received no frames");
        const luma = new Float32Array(first.width * first.height);
        for (let index = 0; index < luma.length; index += 1) {
          const source = index * 4;
          const values = decoded
            .map((image) => {
              const red = at(image.pixels, source) / 255;
              const green = at(image.pixels, source + 1) / 255;
              const blue = at(image.pixels, source + 2) / 255;
              return red * 0.2126 + green * 0.7152 + blue * 0.0722;
            })
            .sort((left, right) => left - right);
          luma[index] = at(values, Math.floor(values.length / 2));
        }
        return { width: first.width, height: first.height, luma };
      }
      function basis({ yaw, pitch }: CameraLook): {
      ...
      function bilinear(image: LumaImage, x: number, y: number): number {
        ...
        const ty = y - y0;
        const top =
          at(image.luma, x0 + y0 * image.width) * (1 - tx) +
          at(image.luma, x1 + y0 * image.width) * tx;
        const bottom =
          at(image.luma, x0 + y1 * image.width) * (1 - tx) +
          at(image.luma, x1 + y1 * image.width) * tx;
        return top * (1 - ty) + bottom * ty;
      }
      function directMetrics(first: LumaImage, second: LumaImage): CloudAnalysis["returned"] {
        let count = 0;
        let absolute = 0;
        let catastrophic = 0;
        for (let y = 0; y < first.height * 0.68; y += 2) {
          for (let x = 0; x < first.width * 0.78; x += 2) {
            const left = at(first.luma, x + y * first.width);
            const right = at(second.luma, x + y * second.width);
      ...
      function layeringMetrics(image: LumaImage): CloudAnalysis["layering"] {
        ...
        for (let y = y0 + 2; y < y1 - 2; y += 2) {
          ...
          for (let x = x0; x < x1; x += 2) {
            const curvature =
              at(image.luma, x + (y - 2) * image.width) -
              at(image.luma, x + y * image.width) * 2 +
              at(image.luma, x + (y + 2) * image.width);
            signed += curvature;
        ...
        for (let x = x0 + 2; x < x1 - 2; x += 2) {
          ...
          for (let y = y0; y < y1; y += 2) {
            const curvature =
              at(image.luma, x - 2 + y * image.width) -
              at(image.luma, x + y * image.width) * 2 +
              at(image.luma, x + 2 + y * image.width);
            signed += curvature;
            absolute += Math.abs(curvature);
        ...
        for (let y = y0; y < y1; y += 1) {
          let total = 0;
          for (let x = x0; x < x1; x += 2) total += at(image.luma, x + y * image.width);
        ...
        const rowAutocorrelations: { readonly lag: number; readonly correlation: number }[] = [];
        for (let lag = 4; lag <= 40; lag += 1) {
          let product = 0;
          let firstEnergy = 0;
          let secondEnergy = 0;
          for (let index = 0; index + lag < residual.length; index += 1) {
            const first = at(residual, index);
            const second = at(residual, index + lag);
      ...
      for (let y = 10; y < baseline.height * 0.68; y += 2) {
        for (let x = 10; x < baseline.width * 0.78; x += 2) {
          ...
          const source = at(baseline.luma, x + y * baseline.width);
          const target = bilinear(rotated, targetX, targetY);
          const blur =
            (at(baseline.luma, x - 8 + y * baseline.width) +
              at(baseline.luma, x + 8 + y * baseline.width) +
              at(baseline.luma, x + (y - 8) * baseline.width) +
              at(baseline.luma, x + (y + 8) * baseline.width)) *
            0.25;
...
async function runWeatherMotion(context: ScenarioContext, arguments_: readonly string[]) {
  const cloudLayeringOnly = arguments_.includes("--cloud-layering-only");
  const extra = arguments_.filter((argument) => argument !== "--cloud-layering-only");
  if (extra.length > 0) throw new Error(`unknown weather-motion options: ${extra.join(" ")}`);
  ...
  const world = await startWorldPreview(context, {
    fixture: {
      ...
      weatherCycleSeconds: cloudLayeringOnly ? 0 : WEATHER_CYCLE_SECONDS,
      weatherFractionAtUnixEpoch: cloudLayeringOnly ? 0.32 : weatherFractionAtUnixEpoch,
      cloudVelocityMetresPerSecond: [0, 0],
    },
  });
  const browser = await BrowserCapability.start(context, {
    warningPattern:
      /panic|unreachable|runtimeerror|wgpu|webgpu|shader|sqlite|opfs|syncaccesshandle|nomodificationallowed|web lock request failed|no persistence leader|persistence .*failed/iu,
  });
  ...
  if (cloudLayeringOnly && cloud.layering.directionalExcess > 0.93) {
    violations.push(`cloud horizontal layer excess was ${cloud.layering.directionalExcess}`);
  }
  if (cloudLayeringOnly && cloud.layering.maximumRowAutocorrelationProminence > 0.25) {
    violations.push(
  ...
  await context.artifacts.writeJson("weather motion report", "report.json", result);
  if (!result.ok) {
    throw new Error(
      `weather-motion found ${violations.length} violation(s): ${violations.join("; ")}`,
    );
  }
  return {
    summary: "Weather motion and cloud stability passed.",
    metrics: {
      rainMedianVelocity,
      cloudCorrelation: cloud.rotation.weightedCorrelation,
      cloudLayeringExcess: cloud.layering.directionalExcess,
    },
    details: result,
  };
}
...
export default defineScenario({
  id: "weather-motion",
  kind: "validation",
  summary: "Validates world-anchored cloud stability, layering, and downward precipitation.",
  uses: {
    world: true,
    browser: true,
    viewport: "browser",
