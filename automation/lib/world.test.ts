...
describe("isolated browser world fixture", () => {
  it("binds matching temporary configuration and preserves omitted renderer defaults", async () => {
    const scenario = defineScenario({
      id: "world-fixture-contract",
      kind: "validation",
      summary: "Exercises isolated typed world fixture generation.",
      uses: { rust: true },
      timeoutMs: 25_000,
      async run(context) {
        ...
        try {
          ...
          expect(client).toContain(
            `endpoint = "ws://127.0.0.1:${fixture.backendPort}${WORLD_PATH}"`,
          );
          expect(client).toContain(
          ...
          expect(service).toContain('source = "procedural-v17"');
        ...
        const defaults = await prepareWorldFixture(context, {
          originPort: 41_235,
          clientPorts: [41_236],
        });
        try {
          const routedClientPath = defaults.clientConfigPaths[0];
          if (routedClientPath === undefined)
            throw new Error("fixture omitted routed client config");
    ...
    await expect(
      runScenario(scenario, [], {
        artifacts: { root: "target/automation-tests", runId: "world-fixture-contract" },
        log: () => {},
      }),
