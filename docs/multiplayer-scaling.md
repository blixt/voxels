...
VXWP v23 supports one unsharded authoritative world with up to 1,024 admitted player sessions. Spatial
...
still render the other five articulated avatars. Per-player `/v23/world` and `/v23/presence` stream and
...
Voxel edits and per-material inventories are native server authority: strict SQLite schema 7 is bound
