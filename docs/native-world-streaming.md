...
VXWP v29 uses the standard `WebSocket` API over loopback. This is the best first transport for
...
1. The browser upgrades `/v29/world`, offering `voxels.world.v29` and the configured local auth token,
...
6. Surface meshes use a separate `VXST` v8 payload containing bounded terrain, water, patch
   boundary-face ranges, explicit parent-height morph closures, child/parent shading-height grids,
   and compact cardinal landscape-horizon profiles. Interactive and horizon ownership activate at
   their own complete-level boundaries; the coarse parent remains resident until its replacement is
   complete, and the renderer derives exact height-matched connectors and lighting morphs from the
   two resident profiles.
7. Every chunk or surface result body is independently Brotli-compressed at quality 2 with a 20-bit
   window. Its mandatory v29 envelope declares the exact uncompressed length; the decoder rejects
...
9. `WorldOpened` also returns a connection-scoped, random presence session token. The browser uses
   it to open `/v29/presence` on a dedicated socket; a token cannot be reused by another world
   connection.
...
12. `EditCommand` carries a typed `Dig` or `Place` action plus the server-issued edit-session ID.
    ...
    empty. VXWP v29 returns the semantic action plus an adaptive sparse-ordinal or dense-bitset
