...
VXWP v44 uses the standard `WebSocket` API over loopback. This is the best first transport for
...
1. The browser upgrades `/v44/world`, offering `voxels.world.v44` and the configured local auth token,
...
5. Virtual-terrain page batches carry `VXTP` v11 pages identified by source, spatial key, revision, and
...
7. Every chunk or surface result body is independently Brotli-compressed at quality 2 with a 20-bit
   window. Its mandatory v44 envelope declares the exact uncompressed length; the decoder rejects
   ...
   unambiguous even when those players use the same operation number. When a fragmented transfer is
   canceled after it starts, the server emits an ordered fragment-abort frame so the receiver can
   immediately release its bounded reassembly slot.
...
9. `WorldOpened` also returns a connection-scoped, random presence session token. The browser uses
   it to open `/v44/presence` on a dedicated socket; a token cannot be reused by another world
   connection.
...
12. `EditCommand` carries a typed `Dig` or `Place` action plus the server-issued edit-session ID.
    ...
    empty. VXWP v44 returns the semantic action plus an adaptive sparse-ordinal or dense-bitset
...
the client never substitutes procedural answers for a remote learned world. VXWP v44 has no
...
Further `VXTP` v11 pages are requested by key, revision, and semantic content fingerprint. They may
...
```toml
[world]
endpoint = "ws://127.0.0.1:9777/v44/world"
presence_endpoint = "ws://127.0.0.1:9777/v44/presence"
subprotocol = "voxels.world.v44"
...
`voxels-worldd` persists last accepted pose, per-material inventory, and sorted sparse edit-chunk
blobs in its manifest-bound native SQLite authority. It retains at most 64 exact operation receipts
per player, plans disjoint 25.6 m horizontal regions concurrently, and streams the same revisions to
every interested browser profile. The browser stores only its local player-ID registry; identity is
still a local claim until authenticated accounts exist.
