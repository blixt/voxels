...
VXWP v28 uses the standard `WebSocket` API over loopback. This is the best first transport for
...
1. The browser upgrades `/v28/world`, offering `voxels.world.v28` and the configured local auth token,
...
7. Every chunk or surface result body is independently Brotli-compressed at quality 2 with a 20-bit
   window. Its mandatory v28 envelope declares the exact uncompressed length; the decoder rejects
...
9. `WorldOpened` also returns a connection-scoped, random presence session token. The browser uses
   it to open `/v28/presence` on a dedicated socket; a token cannot be reused by another world
   connection.
...
12. `EditCommand` carries a typed `Dig` or `Place` action plus the server-issued edit-session ID.
    ...
    empty. VXWP v28 returns the semantic action plus an adaptive sparse-ordinal or dense-bitset
...
```toml
[world]
endpoint = "ws://127.0.0.1:9777/v28/world"
presence_endpoint = "ws://127.0.0.1:9777/v28/presence"
subprotocol = "voxels.world.v28"
