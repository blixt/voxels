...
VXWP v37 uses the standard `WebSocket` API over loopback. This is the best first transport for
...
1. The browser upgrades `/v37/world`, offering `voxels.world.v37` and the configured local auth token,
...
7. Every chunk or surface result body is independently Brotli-compressed at quality 2 with a 20-bit
   window. Its mandatory v37 envelope declares the exact uncompressed length; the decoder rejects
...
9. `WorldOpened` also returns a connection-scoped, random presence session token. The browser uses
   it to open `/v37/presence` on a dedicated socket; a token cannot be reused by another world
   connection.
...
12. `EditCommand` carries a typed `Dig` or `Place` action plus the server-issued edit-session ID.
    ...
    empty. VXWP v37 returns the semantic action plus an adaptive sparse-ordinal or dense-bitset
...
```toml
[world]
endpoint = "ws://127.0.0.1:9777/v37/world"
presence_endpoint = "ws://127.0.0.1:9777/v37/presence"
subprotocol = "voxels.world.v37"
