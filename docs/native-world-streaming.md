...
VXWP v18 uses the standard `WebSocket` API over loopback. This is the best first transport for
...
1. The browser upgrades `/v18/world`, offering `voxels.world.v18` and the configured local auth token,
...
7. Every chunk or surface result body is independently Brotli-compressed at quality 2 with a 20-bit
   window. Its mandatory v18 envelope declares the exact uncompressed length; the decoder rejects
...
9. `WorldOpened` also returns a connection-scoped, random presence session token. The browser uses
   it to open `/v18/presence` on a dedicated socket; a token cannot be reused by another world
   connection.
...
11. Presence ping/pong frames estimate the server clock with an NTP-style four-timestamp offset.
    ...
    past the configured target, and returns to its safe floor if feedback disappears.
...
```toml
[world]
endpoint = "ws://127.0.0.1:9777/v18/world"
presence_endpoint = "ws://127.0.0.1:9777/v18/presence"
subprotocol = "voxels.world.v18"
