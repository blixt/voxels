...
VXWP v26 uses the standard `WebSocket` API over loopback. This is the best first transport for
...
1. The browser upgrades `/v26/world`, offering `voxels.world.v26` and the configured local auth token,
...
7. Every chunk or surface result body is independently Brotli-compressed at quality 2 with a 20-bit
   window. Its mandatory v26 envelope declares the exact uncompressed length; the decoder rejects
...
9. `WorldOpened` also returns a connection-scoped, random presence session token. The browser uses
   it to open `/v26/presence` on a dedicated socket; a token cannot be reused by another world
   connection.
...
```toml
[world]
endpoint = "ws://127.0.0.1:9777/v26/world"
presence_endpoint = "ws://127.0.0.1:9777/v26/presence"
subprotocol = "voxels.world.v26"
