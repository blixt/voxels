...
VXWP v11 uses the standard `WebSocket` API over loopback. This is the best first transport for
...
1. The browser upgrades `/v11/world`, offering `voxels.world.v11` and the configured local auth token,
...
9. `WorldOpened` also returns a connection-scoped, random presence session token. The browser uses
   it to open `/v11/presence` on a dedicated socket; a token cannot be reused by another world
   connection.
...
```toml
[world]
endpoint = "ws://127.0.0.1:9777/v11/world"
presence_endpoint = "ws://127.0.0.1:9777/v11/presence"
subprotocol = "voxels.world.v11"
