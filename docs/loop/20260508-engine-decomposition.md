...
The main game HUD now reports true wall-frame max and hitch buckets with enough attribution to name the expensive LOD chunk. The production idle and smoke-movement paths are currently below the hitch threshold in the shared browser, but the LOD backlog can remain pending for a while because work is now intentionally sliced. The next target is a stronger held-input route harness and a HUD cleanup so stale last-hitch attribution cannot be mistaken for recent-window hitches.
