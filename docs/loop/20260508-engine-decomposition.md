...
The main game HUD now reports true wall-frame max and hitch buckets with enough attribution to name the expensive LOD chunk, and it no longer displays stale hitch causes as if they are recent. The production idle, smoke-movement, and short live-forward benchmark paths are currently below the hitch threshold. The next target is a longer route trace with visual captures/reference diffs enabled, then content/world design can resume with a stronger renderer safety net.
