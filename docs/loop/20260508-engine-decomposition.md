...
The main game HUD now reports true wall-frame max and hitch buckets with enough attribution to name the expensive LOD chunk. The production idle path is currently below the hitch threshold in the shared browser, but the LOD backlog can remain pending for a while because work is now intentionally sliced. The next target is to verify active walking with the same attribution and make sure backlog progress does not regress visible distance or LOD handoff correctness.
