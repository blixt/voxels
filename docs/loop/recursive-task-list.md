...
2. Performance/harness: quantify cold/wide persisted region-summary reuse and add the next manifest/index layer so the far bubble stops depending on one-region-at-a-time discovery.
...
6. Performance/harness: revisit renderer sync/upload, async mesh worker sizing, and summary-driven far-field inputs after persisted region-summary reuse is measurable.
