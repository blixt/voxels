...
/// Diagnostic quality tier for the direct path: 640x360 (1/9 of a 1920x1080 output). Rays are
/// generated on the GPU, so increasing this tier does not add per-frame CPU upload traffic.
const DIRECT_TRACE_WIDTH: u32 = 640;
const DIRECT_TRACE_HEIGHT: u32 = 360;
...
/// Renderer-minted identity for one immutable virtual-terrain publication request.
///
/// The field is private so hosts can retain and return a request but cannot forge one.
...
struct VirtualTerrainPublication {
    ...
    /// or an identical post-edit snapshot from certifying a different request.
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
enum VirtualTerrainPromotionDecision {
    Stale,
    Pending,
    CommitActive(u64),
    PromoteCandidate(u64),
}

fn virtual_terrain_promotion_decision(
    ...
    candidate_generation: Option<u64>,
) -> VirtualTerrainPromotionDecision {
    if publication_request != Some(request) {
        VirtualTerrainPromotionDecision::Stale
    } else if active_matches {
        match active_generation {
            Some(generation) => VirtualTerrainPromotionDecision::CommitActive(generation),
            None => VirtualTerrainPromotionDecision::Pending,
        }
    } else if candidate_certified {
        match candidate_generation {
            Some(generation) => VirtualTerrainPromotionDecision::PromoteCandidate(generation),
            None => VirtualTerrainPromotionDecision::Pending,
        }
    } else {
...
fn exact_virtual_terrain_publication(
    publication: Option<&VirtualTerrainPublication>,
    request: VirtualTerrainRequestId,
) -> Option<&VirtualTerrainPublication> {
    publication.filter(|publication| publication.request == request)
}

fn mint_virtual_terrain_request(next_request: &mut u64) -> VirtualTerrainRequestId {
    let request = VirtualTerrainRequestId(*next_request);
    *next_request = next_request.wrapping_add(1).max(1);
    request
}

fn take_invalidated_virtual_terrain_publication(
    publication: &mut Option<VirtualTerrainPublication>,
    change: &WorldChange,
) -> bool {
    if publication
        .as_ref()
        .is_some_and(|publication| change.invalidates(&publication.revisions))
    {
        *publication = None;
        true
    } else {
...
pub enum VirtualTerrainRendererError {
    ...
    RevisionFence(RevisionFenceError),
...
impl std::fmt::Display for VirtualTerrainRendererError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::RevisionFence(error) => {
                write!(formatter, "invalid world revision fence: {error:?}")
            }
...
impl From<RevisionFenceError> for VirtualTerrainRendererError {
    fn from(error: RevisionFenceError) -> Self {
        Self::RevisionFence(error)
    }
}
...
fn virtual_microvoxel_gpu_quads(
    ...
    if let Some(water) = water {
        ...
        for z in 0..cells {
            for x in 0..cells {
                // Level-zero water is a 10 cm voxel-cell surface, just like level-zero ground. The
                // lower-X/Z sample owns the complete cell; consulting four corners would emit
                // partial shoreline triangles that cannot partition cleanly against a canonical
                // voxel top face.
                if let Some(height) = heightfield.water[x + z * edge] {
                    flat_heights[x + z * cells] = Some(lattice_height(height)?);
                }
            }
        }
...
fn virtual_terrain_surface_slice(
    ...
    MeshSlice {
        ...
        bounds_max,
        render_layer,
        ownership: CanonicalSliceOwnership::Volume,
    }
}
...
fn canonical_gpu_quads_partitioned(
    world_origin: [i32; 3],
    quads: &[Quad],
    ...
    let base_quads = quads
        ...
        .map(|quad| canonical_gpu_quad(world_origin, quad))
        .collect::<Vec<_>>();
    // Split the two semantic source ranges independently, concatenate them, and only then run one
    // conformity pass. Every T-junction therefore sees every neighbor while the returned group
...
struct MeshSlice {
    ...
    bounds_max: glam::Vec3,
    render_layer: RenderLayer,
    ownership: CanonicalSliceOwnership,
}
...
impl Renderer {
    ...
    fn screenshot_reproduction_metadata(
        ...
        let virtual_terrain_manifest =
            screenshot_virtual_terrain_manifest_json(ScreenshotVirtualTerrainManifestContext {
                ...
                committed_envelope: self.committed_virtual_terrain_envelope(),
                committed_cut: self.committed_virtual_terrain_cut(),
    ...
    pub fn upload_virtual_terrain_replacement_group(
        ...
        let logical_replacement_missing = pages
            .iter()
        ...
        // Reclaim unrelated travel history once. Each sibling is then prepared from its exact CPU
        // geometry; an allocation failure rolls the complete group back below. Directory
        // `source_geometry_bytes` is an intentionally conservative transport bound and must not be
        // used as a GPU admission size because it can reject heightfields whose real mesh fits.
        self.retain_virtual_terrain_gpu_pages(std::iter::empty())?;
    ...
    pub fn select_virtual_terrain_cut(
        ...
        let (oracle_view, cut) = loop {
            ...
            match capacity {
                ...
                Err(VirtualTerrainRendererError::SelectedPageMissingGpu(_)) => {
                    // The hierarchy can prove a complete target after its expanded geometry has
                    // been evicted. Rehydrate that exact target first; once every selected source
                    // record exists, the ordinary per-stream capacity proof runs before freezing a
                    // publication. Missing geometry is appended to this cut's demand below.
                    break (candidate_view, candidate);
                }
    ...
    /// Rebuilds expanded draw geometry directly from validated logical pages already owned by the
    /// hierarchy.
    ///
    /// Hierarchy/coherence residency and GPU geometry residency are deliberately independent. The
    /// former must converge monotonically while a legal cut is discovered; the latter is an
    /// evictable derived cache. Routing a selected logical page back through `requested_pages`
    /// conflates those states and can deadlock publication behind a redundant network request even
    /// though the complete `TerrainPageV1` is already resident here.
    ///
    /// The caller bounds work per frame. Successfully rebuilt pages remain candidate-only until
    /// the ordinary immutable publication proof promotes the complete selected cut.
    pub fn rebuild_selected_virtual_terrain_gpu_pages(
        &mut self,
        cut: &VirtualTerrainCut,
        maximum_pages: usize,
        ...
        for page in pages {
            rebuilt +=
                usize::from(self.upload_virtual_terrain_page_with_seams(page, [false; 4], false)?);
        }
        Ok(rebuilt)
    }
    ...
    /// change while the frozen cut is encoded and certified. Every selected page contributes its
    /// exact accepted revision to the immutable fence. Even an already-matching active bank goes
    /// through this request path; preparation never commits presentation metadata.
    pub fn prepare_virtual_terrain_publication(
        &mut self,
    ) -> Result<Option<VirtualTerrainRequestId>, VirtualTerrainRendererError> {
        if self.virtual_terrain_publication.is_some() {
            return Ok(None);
        }
    ...
    pub fn commit_virtual_client_view(
        ...
        self.invalidate_virtual_terrain_desired_plan();
        self.discard_unselected_virtual_terrain_logical_pages();
        Ok(receipt)
    }
    ...
    fn discard_virtual_terrain_publication_resources(&mut self) {
        self.virtual_terrain_gpu.discard_candidate();
    ...
    ///
    /// Validated CPU pages and their replacement-coherence proofs remain in
    /// [`VirtualTerrainHierarchy`]. Removing them together with draw geometry made legal-cut
    /// discovery non-monotonic: admitting one balance group could evict a previously proved group,
    /// causing the same groups to alternate forever. The encoded shell cache remains the source for
    /// rebuilding only the expanded geometry selected by a later cut.
    pub fn retain_virtual_terrain_gpu_pages(
        &mut self,
        keep: impl IntoIterator<Item = TerrainPageKey>,
    ) -> Result<usize, VirtualTerrainRendererError> {
    ...
    /// Ends one monotonic transition epoch after its complete cut promotes.
    ///
    /// Logical pages are retained while a replacement/balance component is unresolved so sibling
    /// groups cannot oscillate. Once the immutable cut commits, pages outside that owner and its
    /// resident ancestor spine are only historical staging data: keeping them forever turns
    /// ordinary stationary refinement into an unbounded decoded-page heap. The spine is mandatory
    /// because every subsequent cut traversal starts at its active roots; retaining leaves alone
    /// would make the next frame refetch its own committed ancestry. Directory identities remain
    /// registered and the encoded shell cache can supply a future transition.
    fn discard_unselected_virtual_terrain_logical_pages(&mut self) -> usize {
        let selected = self
            .committed_virtual_terrain_cut()
            .map(|cut| cut.selected_pages.clone())
            .unwrap_or_default();
        let keep = resident_virtual_terrain_ancestry(&selected, |key| {
            self.virtual_terrain.resident_page(key).is_some()
        });
        let remove = self
            .virtual_terrain
            ...
            .collect::<Vec<_>>();
        for key in &remove {
            let removed = self.virtual_terrain.remove_page(*key);
    ...
    fn prepare_canonical_chunk_upload(
        ...
        let opaque_update = if opaque_count == 0 {
            ...
            if opaque_surface_count != 0 {
                slices.push(MeshSlice {
                    relative_offset: 0,
                    size: opaque_surface_count * quad_bytes,
                    quad_count: opaque_surface_count,
                    bounds_min: min,
                    bounds_max: max,
                    render_layer: RenderLayer::Opaque,
            ...
            let prepared = self.prepare_mesh_sliced(key, &opaque_quads, slices)?;
            Some(prepared)
        };
    ...
    pub fn render(
        ...
        if let (Some(identity), Some(work)) = (virtual_candidate, virtual_candidate_work) {
            ...
            match self.virtual_terrain_gpu.encode_candidate(
                ...
                Ok(VirtualTerrainCandidateEncodeOutcome::Encoded(generation)) => {
                    ...
                    if let Some(publication) = self.virtual_terrain_publication.as_mut() {
                        publication.candidate_generation = Some(generation);
                    }
        ...
        ) = self.committed_virtual_terrain_envelope().map_or(
            (0, 0, 0, 0, 0, [0; 2], [0; 2]),
            |envelope| {
                let certificate = self
                    .committed_virtual_terrain_certificate()
                    .unwrap_or_default();
                let locus = envelope.locus();
                (
                    envelope.fingerprint(),
...
fn resident_virtual_terrain_ancestry(
...
mod tests {
    ...
    fn test_publication(
        ...
        revisions: WorldRevisionFence,
        candidate_generation: Option<u64>,
    ) -> VirtualTerrainPublication {
        VirtualTerrainPublication {
            request,
            cut: virtual_cut_with_selected(vec![TerrainPageKey::surface(0, 0, 0)]),
            envelope,
            certificate: PresentationCoverageCertificate {
                complete: true,
                exact_coverage: 1,
    ...
    #[test]
    fn canonical_surface_and_volume_partition_share_one_t_junction_conformity_pass() {
        let top = |origin, extent| Quad {
            origin,
            face: 2,
            extent,
    ...
    #[test]
    fn publication_promotion_requires_the_exact_renderer_request() {
        let request_a = VirtualTerrainRequestId(11);
        let request_b = VirtualTerrainRequestId(12);
        assert_eq!(
            virtual_terrain_promotion_decision(
                Some(request_a),
                request_a,
                false,
                None,
            ...
            VirtualTerrainPromotionDecision::Pending,
            "candidate construction must never self-promote",
        );
        assert_eq!(
            virtual_terrain_promotion_decision(
                Some(request_a),
            ...
            VirtualTerrainPromotionDecision::Stale,
            "a certified generation cannot be claimed by a different request",
        );
        assert_eq!(
            virtual_terrain_promotion_decision(
                Some(request_a),
            ...
            VirtualTerrainPromotionDecision::CommitActive(7),
            "an already matching active bank still needs the exact explicit request",
        );
        assert_eq!(
            virtual_terrain_promotion_decision(
                Some(request_a),
            ...
            VirtualTerrainPromotionDecision::PromoteCandidate(19),
        );
        ...
        let publication = test_publication(
            request_a,
            envelope,
            WorldRevisionFence::new([], []).expect("empty test fence"),
            Some(19),
        );
        let candidate = Some(publication.clone());
        assert!(exact_virtual_terrain_publication(candidate.as_ref(), request_b).is_none());
        assert_eq!(
            candidate,
            Some(publication),
            "a wrong request must be a strict state no-op",
        );
    }

    #[test]
    fn explicit_active_match_commits_one_aggregate_receipt() {
        let request = VirtualTerrainRequestId(31);
        let envelope = crate::virtual_terrain::PresentationEnvelopeCache::default().resolve(
            [0.0, 0.0, 0.0],
            0.0,
            20.0,
    ...
    fn presentation_reversal_keeps_a_frozen_request_while_it_still_covers_the_target() {
        ...
        assert!(candidate.is_none());
        assert_eq!(
            virtual_terrain_promotion_decision(None, request_b, false, None, true, Some(99)),
            VirtualTerrainPromotionDecision::Stale,
        );
    }
    ...
    fn world_change_invalidates_candidates_but_retains_the_immutable_presented_owner() {
        ...
        let observed = authoritative
            .observe_world_change_batch(
                &[voxels_world::VoxelCoord::new(
                    affected_chunk.x * CHUNK_EDGE as i32,
                    affected_chunk.y * CHUNK_EDGE as i32,
                    affected_chunk.z * CHUNK_EDGE as i32,
                )],
                2,
                &[affected_chunk],
            )
            .expect("representable edit change");
        let changed = observed.change();
        let affected_page = changed.terrain_pages()[0].key;
        let affected_fence = WorldRevisionFence::new(
            [],
            [TerrainPageRevision {
                key: affected_page,
                revision: 1,
        ...
        .expect("affected test fence");
        let unrelated_fence = WorldRevisionFence::new(
            [],
            [TerrainPageRevision {
                key: TerrainPageKey::surface(0, 500, 500),
                revision: 1,
            }],
        )
        .expect("unrelated test fence");
        ...
        let mut candidate = Some(test_publication(
            VirtualTerrainRequestId(51),
            envelope.clone(),
            affected_fence.clone(),
            Some(12),
        ));
        let unrelated_publication = test_publication(
            VirtualTerrainRequestId(52),
            envelope.clone(),
    ...
    #[test]
    fn same_owner_invalidation_mints_a_fresh_request() {
        let mut next_request = 61;
        let first = mint_virtual_terrain_request(&mut next_request);
        let second = mint_virtual_terrain_request(&mut next_request);
        assert_ne!(first, second);
        assert_eq!(first, VirtualTerrainRequestId(61));
        assert_eq!(second, VirtualTerrainRequestId(62));
    }
    ...
    #[test]
    fn committed_leaf_retention_keeps_its_resident_traversal_spine() {
        ...
        let unrelated = TerrainPageKey {
            level: 0,
            coord: [99, 99, 99],
        };
        let resident = BTreeSet::from([leaf, parent, root, unrelated]);

        assert_eq!(
            resident_virtual_terrain_ancestry(&[leaf], |key| resident.contains(&key)),
            BTreeSet::from([leaf, parent, root]),
            "the next cut must be able to traverse from its root to the committed leaf without refetching"
        );
    }
    ...
    #[test]
    fn height_surface_ownership_hides_only_the_certified_surface_slice() {
        let root = TerrainPageKey::surface(TERRAIN_COVERAGE_ROOT_LEVEL, -1, 2);
        let ownership = VirtualTerrainOwnership::from_cut(
            &virtual_cut_with_selected(root.refinement_children().unwrap()),
            |_| {
        ...
        assert!(
            ...
            "canonical meshes must not redraw the certified virtual height surface"
        );
        assert!(
            !canonical_bounds_owned_by_virtual(
                (0, -1, 0, 66),
        ...
        assert!(
            ...
            "height surfaces must preserve caves, overhangs, features, and edits"
        );
    }
