...
pub const WORLD_WEBSOCKET_PATH: &str = "/v44/world";
pub const PRESENCE_WEBSOCKET_PATH: &str = "/v44/presence";
pub const WORLD_WEBSOCKET_PROTOCOL: &str = "voxels.world.v44";
...
impl PendingGenerationBatch {
    ...
    fn is_cancelled(&self) -> bool {
        self.job.tracked.is_cancelled()
    }

    fn finish(self) {
        self.job.tracked.finish();
    }
}
...
async fn serve_virtual_terrain_pages(
    ...
    for identity in request.batch.pages {
        let root = if identity.key.is_surface() {
            if identity.key.level == TERRAIN_COVERAGE_ROOT_LEVEL {
                Some(identity.key)
            } else {
                identity
                    .key
...
async fn generate_single_flight_products(
    ...
    let result = tokio::task::spawn_blocking(move || {
        let emit = |result: Result<EncodedProduct, String>| {
        ...
        GenerationRequest::Chunks { request, .. } => {
            let items = products
                ...
                .map(|product| match product.as_ref() {
                    EncodedProduct::Chunk(item) => item,
                })
                .collect::<Vec<_>>();
        ...
        coords.into_iter().zip(snapshot.revisions).zip(result.items)
    {
        ...
        let item_result = match item.result {
            Ok(WorldProduct::Chunk(mut chunk)) => {
                ...
                chunk.meshing_edits = snapshot
                    .edits
                    .meshing_edit_envelope(coord)
...
mod tests {
    ...
    use voxels_world::protocol::{
        ...
        PresenceOpened, PresenceSessionId, decode_chunk_batch_result, decode_edit_commit,
        decode_error, decode_frame_fragment, decode_frame_fragment_abort, decode_presence_delta,
        decode_presence_opened, decode_world_opened, edit_commit_kind, encode_chunk_batch,
        encode_edit_command, encode_open_presence, encode_open_world, encode_player_pose,
    ...
    #[tokio::test]
    async fn virtual_terrain_page_handler_restreams_evicted_surface_coverage_root() {
        let source: Arc<dyn WorldSourceEngine> = Arc::new(ProceduralWorldSource::new(20));
        let edits = EditAuthority::in_memory(
            voxels_world::WorldId::from_bytes([9; 16]),
            source.as_ref(),
            8,
        )
        .expect("edits");
        ...
        let region = authority
            ...
            .await
            .expect("coverage region");
        let node = region.directory.node(root).expect("coverage root node");
        let identity = voxels_world::TerrainPageTransferIdentity {
            key: node.key,
            revision: node.revision,
            content_fingerprint: node.content_fingerprint,
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        request.headers_mut().insert(
            ...
            HeaderValue::from_static("voxels.world.v44, test-local-token"),
        );
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let first_coord = ChunkCoord::new(-400, 0, 200);
        let second_coord = ChunkCoord::new(700, 0, -600);
        for (socket, coord) in [(&mut first, first_coord), (&mut second, second_coord)] {
            ...
            socket
                .send(ClientMessage::Binary(encode_chunk_batch(&request)?.into()))
                .await?;
        }
        let first_result = decode_chunk_batch_result(&next_client_binary(&mut first).await?)?;
        let second_result = decode_chunk_batch_result(&next_client_binary(&mut second).await?)?;
    ...
    #[tokio::test]
    async fn digging_converges_editor_observer_inventory_chunks_and_halos()
    -> Result<(), Box<dyn std::error::Error>> {
        let mut config = test_config();
