...
export class ProceduralWorldGenerator {
  ...
  generateChunkAtLod(cx: number, cy: number, cz: number, lodLevel: number): GeneratedChunk {
    if (lodLevel === 0) {
      return this.generateChunk(cx, cy, cz);
    }
    const stride = 1 << lodLevel;
    ...
    // Sample at the center of each stride cell. This preserves biome blending
    // continuity and cave generation patterns. The noise functions are resolution-
    // independent so coarser sampling naturally produces a simplified but
    // consistent version of the same terrain.
    const halfStride = stride >> 1;
    for (let z = 0; z < this.chunkSize; z += 1) {
      const worldZ = originZ + z * stride + halfStride;
      const rowOffset = z * this.chunkSize;
      for (let x = 0; x < this.chunkSize; x += 1) {
        this.fillColumnState(originX + x * stride + halfStride, worldZ, columnState);
        this.writeChunkColumnState(scratch, x + rowOffset, columnState);
      }
    }
    ...
    const maxRelevantY = Math.min(originY + worldSize - 1, columnMaxTopY);
