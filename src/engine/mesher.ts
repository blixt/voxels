...
const meshMaterialLutByWorld = new WeakMap<ResidentChunkWorld, MeshMaterialLut>();
...
function getWorldMeshMaterialLut(world: ResidentChunkWorld): MeshMaterialLut {
  const cached = meshMaterialLutByWorld.get(world);
  if (cached) {
    return cached;
  }
  const palette = readWorldPalette(world) ?? Array.from(
    { length: MAX_MATERIAL_INDEX },
    (_, materialIndex) => world.getPaletteColor(materialIndex),
  );
  const lut = createMeshMaterialLut(palette, (materialIndex) => world.isWaterMaterial(materialIndex));
  meshMaterialLutByWorld.set(world, lut);
  return lut;
}

function readWorldPalette(world: ResidentChunkWorld): readonly number[] | null {
  const maybePalette = (world as { readonly palette?: unknown }).palette;
  return Array.isArray(maybePalette) ? maybePalette as readonly number[] : null;
}
...
export function buildChunkMesh(world: ResidentChunkWorld, cx: number, cy: number, cz: number): ChunkMeshData {
  ...
  const input = createOpaqueChunkMeshingInput(world, cx, cy, cz);
  if (!input) {
  ...
  return buildChunkMeshFromOpaqueGeometry(
    world,
    cx,
    cy,
    cz,
    buildOpaqueChunkMeshFromInput(input, getWorldMeshMaterialLut(world)),
  );
}
...
function appendQuadToArray(
  ...
  const lengthKey = "waterQuadLength";
